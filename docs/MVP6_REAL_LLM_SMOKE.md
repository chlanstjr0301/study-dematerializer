...
# Step C (session creation)가 timeout되면 timeout을 늘려 재시도:
...
# Smoke test (--http-timeout-seconds 600 권장: 원격 서버 latency 고려)
python scripts/smoke_real_llm.py --allow-real-llm --base-url http://127.0.0.1:8000 --http-timeout-seconds 600
...
| 10 | smoke script TimeoutError | HTTP timeout (기본 300s) | smoke script 출력 | `--http-timeout-seconds 600` |
